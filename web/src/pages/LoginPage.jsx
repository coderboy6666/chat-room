...
/**
 * 1、创建登录界面
 * 2、获取store中的islogging和login变量处理登录请求
 * 3、使用useState创建formData{email, password}用来双向绑定表单数据
 * 4、创建checkForm函数用来检查输入数据的正确性
 * 5、创建submitForm函数用来调用login函数提交表单
 * 6、使用useState创建showPassword变量决定密码的渲染形式
 */
const LoginPage = () => {
    ...
    const checkForm = () => {
        ...
        if (formData.password.length < 6) return toast.error("密码长度不少于6位！！");
        return true;
    }
